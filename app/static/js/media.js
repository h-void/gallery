...
function renderGrid() {
  ...
  const html = tagResultsHtml + (state.duplicatesOnly ? renderDuplicateGroups(items) : renderItemCards(items, 0));
...
function bindGridEvents() {
  ...
  $$('#grid [data-favorite]').forEach(btn => {
    if (btn.dataset.favoriteBound === '1') return;
    btn.dataset.favoriteBound = '1';
    btn.addEventListener('click', e => {
      e.stopPropagation();
      const item = state.allItems.find(row => row.id === parseInt(btn.dataset.favorite));
...
async function deleteMediaItem({filePath, itemId, fileName = '', button = null, closeLightboxAfter = false, skipConfirm = false, onError = null, renderAfter = true, toastOnSuccess = true}) {
  ...
  if (!skipConfirm && !confirm(`确定要将文件「${fileName || itemId}」移至回收站吗？\n文件将安全移入系统回收站，可随时恢复。`)) return false;
  ...
  try {
    ...
    if (toastOnSuccess) toast('文件已移至回收站', 'success');
    return true;
  } catch (e) {
