...
fn load(conn: &Connection) -> Result<Value> {
    ...
    Ok(archive_format::normalize_settings(&value)?)
}
...
pub fn set_folder_rename_format_settings(
    ...
    conn.execute("INSERT INTO app_settings(key,value,updated_at) VALUES(?,?,strftime('%s','now')) ON CONFLICT(key) DO UPDATE SET value=excluded.value, updated_at=excluded.updated_at", params![archive_format::ARCHIVE_FORMAT_SETTINGS_KEY, normalized.to_string()])?;
...
pub fn preview_folder_rename_template(
    ...
    let selected = if let Some(ids) = plan_ids {
        ids.iter()
            .map(|id| id.to_string())
            .collect::<Vec<_>>()
            .join(",")
    ...
    Ok(
        json!({"ok": can_apply, "artist_id": artist_id, "profile": profile, "format_source": source, "plans": previews, "conflicts": conflicts, "can_apply": can_apply}),
    )
}
