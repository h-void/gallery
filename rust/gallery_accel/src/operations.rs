...
fn operation_move_history(
    ...
    let mut stmt = conn.prepare(
        ...
        FROM move_history
        ORDER BY COALESCE(reverted_at, applied_at, created_at) DESC, id DESC
        LIMIT ?
        ",
    )?;
    ...
    for row in rows {
        ...
        let reverted = status == "reverted";
        let (source, target) = if reverted {
            (new_path, old_path)
        } else {
            (old_path, new_path)
        };
        ...
        let updated_items = if matches!(status.as_str(), "applied" | "reverted") {
            1
        } else {
            0
        };
        history.push(json!({
            ...
            "kind": "move",
            "status": status,
            "at": reverted_at.or(applied_at).unwrap_or(created_at),
            "artist_id": artist_id,
