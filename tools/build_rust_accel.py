...
def _windows_path_as_wsl_path(path: Path) -> str:
    ...
    if not drive:
        raise RuntimeError(f"cannot map Windows path to WSL path: {path}")
...
def _windows_root_as_wsl_path() -> str:
    return _windows_path_as_wsl_path(ROOT)
...
def build_rust_accel(output: Path = DEFAULT_OUTPUT) -> Path:
    ...
    _ensure_ort_libs()
    if platform.system().lower() == "windows" and not os.environ.get(
        "GALLERY_RUST_BUILD_HOST", ""
    ).strip().lower() in ("1", "true", "yes") and _wsl_has_podman():
        binary = _build_clean_via_podman_wsl(_windows_root_as_wsl_path(), output)
        build_target = "x86_64-unknown-linux-gnu (clean WSL)"
    else:
        used_target = _run_cargo_release_build()
        binary = _resolve_binary(used_target=used_target)
        build_target = used_target
    ...
    print(f"built artifact: {binary} (target={build_target}) -> {output}")
    return output
