...
function syncItemFilterControls() {
  $('#itemSort').value = state.itemSort;
  $('#itemDateFrom').value = state.itemDateFrom;
  $('#itemDateTo').value = state.itemDateTo;
  $('#itemDateFrom').max = state.itemDateTo;
  $('#itemDateTo').min = state.itemDateFrom;
}

function applyItemFilterControls() {
  const dateFrom = $('#itemDateFrom');
  const dateTo = $('#itemDateTo');
  const invalid = dateFrom.value && dateTo.value && dateFrom.value > dateTo.value;
  dateFrom.setCustomValidity(invalid ? '开始日期不能晚于结束日期' : '');
  dateTo.setCustomValidity(invalid ? '结束日期不能早于开始日期' : '');
  if (invalid) {
...
function bindEvents() {
  ...
  $('#itemDateFrom').addEventListener('change', applyItemFilterControls);
  $('#itemDateTo').addEventListener('change', applyItemFilterControls);
  $('#itemFiltersReset').addEventListener('click', () => {
  ...
  if (archiveProfileDeleteBtn) archiveProfileDeleteBtn.addEventListener('click', deleteArchiveProfile);
  ...
  if (archivePlanList) archivePlanList.addEventListener('click', e => {
    ...
    if (!target) return;
    const save = target.closest('[data-archive-plan-save]');
    if (save && archivePlanList.contains(save)) return void saveArchivePlanTarget(save.dataset.archivePlanSave);
