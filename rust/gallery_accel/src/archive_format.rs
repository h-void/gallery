...
//! rendering separate from database and filesystem work so previews and
//! generated plans use the same validation.
...
pub(crate) fn normalize_settings(input: &Value) -> Result<Value> {
    ...
    // The UI and automatic archive flow share one editable Default rule. Keep
    // the active legacy rule's values during migration, but discard profiles
    // and artist overrides that could otherwise make automatic targets differ.
    let active_profile_id = object
        ...
        .trim()
        .to_string();
    let selected = profiles_array
        .iter()
        .find(|profile| profile["id"].as_str() == Some(active_profile_id.as_str()))
        .or_else(|| {
            profiles_array
                .iter()
                .find(|profile| profile["id"].as_str() == Some(DEFAULT_PROFILE_ID))
        })
        .unwrap_or(&profiles_array[0]);
    let mut profile = normalize_profile(selected)?;
    profile["id"] = Value::String(DEFAULT_PROFILE_ID.to_string());
    profile["name"] = Value::String("Default".to_string());
...
pub(crate) fn profile_for_artist(settings: &Value, artist_id: i64) -> Result<(Value, String)> {
    ...
    Ok((profile, DEFAULT_PROFILE_ID.to_string()))
}
...
pub(crate) fn profile_by_id(settings: &Value, profile_id: &str) -> Result<Value> {
    ...
    settings["profiles"]
        ...
        .and_then(|profiles| profiles.first())
        .cloned()
        .ok_or_else(|| anyhow!("Default archive format profile is not configured"))
}
...
mod tests {
    ...
    #[test]
    fn settings_migrate_legacy_profiles_to_one_default_rule() {
        ...
        assert_eq!(profile["template"], "{title}");
