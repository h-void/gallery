...
use gallery_accel::{
    ...
    ignore_move_candidate_response, item_detail_response, items_page_cursor_query_response,
    items_page_query_response, list_folder_renames, list_media_root_directories,
    ...
    operation_history_response, operation_log_response, preview_artist_folder_move,
    preview_folder_rename_template, preview_jpeg_allowed, propagate_hash_tags_response,
    rebuild_character_index, recheck_plan, recognize_character_native_topk_with_roots,
    reconfirm_plan, recycle_entries_response, reindex_artist_links,
    resolve_existing_scan_candidate_response_with_roots, resolve_scan_scope, restore_recycle_entry,
    run_full_library_scan, run_hash_batch_with_roots, run_scan, scan_candidates_response,
    serve_file_response, serve_text, serve_transcoded_hls, serve_transcoded_hls_segment,
...
fn folder_archive_failed_plans(conn: &rusqlite::Connection) -> anyhow::Result<i64> {
    ...
    Ok(conn.query_row(
        "SELECT COUNT(*) FROM folder_rename_plans WHERE status='manual_review'",
        [],
        |row| row.get(0),
    )?)
}
...
async fn api_folder_renames_put(
    ...
    upsert_folder_rename_plans(&conn, body.artist_id, &body.plans)
        .map(Json)
        .map_err(to_folder_rename_input_http_error)
}
...
fn to_folder_rename_input_http_error(error: anyhow::Error) -> (StatusCode, Json<Value>) {
    let message = error
        .chain()
        .map(|cause| cause.to_string())
        .collect::<Vec<_>>()
        .join(": ");
    if message.contains("invalid folder rename plan:") {
        return (
            StatusCode::BAD_REQUEST,
            Json(json!({"ok": false, "reason": "invalid_plan", "error": message})),
        );
    }
    to_http_error(error)
}
...
mod tests {
    ...
    #[tokio::test]
    async fn folder_plan_validation_returns_bad_request() {
        let (_dir, app) = tag_test_app();
        let (status, body) = json_response(
            &app,
            Request::builder()
                .method(Method::PUT)
                .uri("/api/folder-renames")
                .header("content-type", "application/json")
                .body(Body::from(
                    r#"{"artist_id":1,"plans":[{"source_folder":"source","status":"executed"}]}"#,
                ))
                .unwrap(),
        )
        .await;
        ...
        assert_eq!(status, StatusCode::BAD_REQUEST);
        assert_eq!(body["reason"], "invalid_plan");
    }
}
