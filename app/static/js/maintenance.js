...
async function loadMlRuntime(options = {}) {
  const render = options.render !== false;
  const updateState = options.updateState !== false;
  const fetchOptions = options.signal ? {signal: options.signal} : {};
  try {
...
function renderMlRuntime() {
  ...
  const plannedProvider = {
    ...
    openvino: 'OpenVINO',
    cpu: 'CPU',
  }[status.planned_provider] || status.planned_provider || '未知';
  const actualProvider = {
    ...
    none: '无可用会话',
  }[status.actual_provider] || status.actual_provider || '未初始化';
  const requestedProvider = {
    auto: '自动',
    cuda: 'CUDA',
    openvino: 'OpenVINO',
    cpu: 'CPU',
  }[status.requested_provider] || status.requested_provider || '未知';
...
async function saveMlDownloadSource() {
