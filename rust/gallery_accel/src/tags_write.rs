...
pub fn update_tag(
    ...
    tx.commit().context("commit tag update")?;
...
pub fn delete_tag(conn: &Connection, artist_id: i64, tag_id: i64) -> Result<Option<Value>> {
    let tx = Transaction::new_unchecked(conn, TransactionBehavior::Immediate)
        .context("begin tag deletion")?;
    let tag_row: Option<Value> = tx
        .query_row(
    ...
    tx.commit().context("commit tag deletion")?;
    Ok(Some(result))
}
...
fn items_grouped_by_artist(conn: &Connection, item_ids: &[i64]) -> Result<BTreeMap<i64, Vec<i64>>> {
