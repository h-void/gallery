...
fn count_media_files(directory: &Path) -> usize {
    ...
    for entry in entries.flatten() {
        ...
        if entry.file_type().map(|t| t.is_file()).unwrap_or(false)
            && media_type_for_file(&name).is_some()
        {
            count += 1;
        }
    }
...
fn walk_artist(
    ...
    for entry in WalkDir::new(scan_root).into_iter().filter_entry(|e| {
        ...
        )) = existing
        {
            ...
            let existing_candidate: Option<(i64, i64, f64, String, String, String)> = conn
                .query_row(
                    "SELECT id, file_size, file_mtime, content_hash, hash_status, status
            ...
            {
                let same = old_size == file_size && (old_mtime - file_mtime).abs() < 1.0;
                let keep_hash = same && old_hash_status == "done" && !old_hash.is_empty();
                let next_status = if old_status == "previewed" || !same {
                    "pending"
                } else {
                    &old_status
                };
                if old_status == "previewed" || !same {
                    conn.execute(
                        "UPDATE move_candidates SET status='superseded', resolved_at=strftime('%s','now')
                ...
                conn.execute(
                    "UPDATE scan_candidates
                     ...
                     WHERE id=?",
                    params![
                        scan_id, artist_id, fname, file_size, file_mtime, folder_name, date_str,
                        is_archive, media_type,
                        if keep_hash { old_hash.clone() } else { String::new() },
                        if keep_hash { "done" } else { "pending" },
                        st_dev, st_ino, next_status, candidate_id,
                    ],
                )?;
                new_candidates += i64::from(old_status == "previewed" || !same);
                (
                    if keep_hash { old_hash } else { String::new() },
                    if keep_hash {
                        "done".into()
                    } else {
                        "pending".into()
