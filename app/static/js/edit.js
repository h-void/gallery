...
async function deleteSelectedMediaItems() {
  ...
  if (!window.confirm(`确定要将选中的 ${items.length} 项移入回收站吗？`)) return;
  ...
  try {
    ...
    if (failedItemIds.length) {
      ...
      toast(`已成功将 ${deletedItemIds.length} 项移至回收站`, 'success');
    }
  } finally {
...
function selectedEditDateItems() {
  const ids = new Set([...state.selectedIds].map(Number));
  return (state.allItems || []).filter(item => ids.has(Number(item.id)));
}

function effectiveRawDateOf(item) {
  const manual = String(item.manual_date || '').trim();
  if (manual) return manual;
  const detected = String(item.detected_date || '').trim();
...
function syncEditDatePrecisionInputs() {
  ...
  if (!precision || !monthInput || !dayInput) return;
  const useDay = precision.value === 'day';
  monthInput.hidden = useDay;
  dayInput.hidden = !useDay;
  if (useDay && monthInput.value && !dayInput.value) dayInput.value = monthInput.value + '-01';
...
function renderEditDateControl() {
  ...
  const monthInput = $('#editDateMonth');
  const dayInput = $('#editDateDay');
  const items = selectedEditDateItems();
  const disabled = items.length === 0 || Boolean(state.readOnlyMode);
  ...
  const first = effectiveRawDateOf(items[0]);
  const shared = Boolean(first) && items.every(item => effectiveRawDateOf(item) === first);
  const precision = $('#editDatePrecision');
  if (shared) {
    const isDay = /^\d{4}-\d{2}-\d{2}$/.test(first);
    if (precision) precision.value = isDay ? 'day' : 'month';
    if (monthInput) monthInput.value = first.slice(0, 7);
    if (dayInput) dayInput.value = first;
    if (summary) summary.textContent = '';
    syncEditDatePrecisionInputs();
  } else {
    if (monthInput) monthInput.value = '';
    if (dayInput) dayInput.value = '';
    if (summary) summary.textContent = items.length > 1 ? '' : '未识别日期';
  }
}
...
function editDateEnteredValue() {
  ...
  const dayInput = $('#editDateDay');
  if (!precision) return '';
  if (precision.value === 'day') return String(dayInput.value || '').trim();
...
async function applyItemDateBatch(manualDate) {
  ...
  setActionBusy('edit-date-batch', '', true);
  const gridScrollAnchor = captureGridScrollAnchor();
  try {
    const result = await API.putJson('/api/items/date', {
      artist_id: artistId,
      item_ids: ids,
      manual_date: manualDate,
    });
    logUiAction('edit_date_apply', {
      mode: manualDate ? 'set' : 'reset',
      requested_count: ids.length,
      item_ids: ids,
      updated: result?.updated || 0,
      manual_date: manualDate || null,
      plans_invalidated: result?.plans_invalidated || 0,
    });
...
function aggregateCharacterTagSuggestions(results) {
  ...
  results.forEach(result => {
    ...
    if (existing) {
      ...
      if (Number.isFinite(itemId) && !existing.item_ids.includes(itemId)) existing.item_ids.push(itemId);
    ...
    byName.set(key, {
      ...
      confidence,
      count: 1,
      item_ids: Number.isFinite(itemId) ? [itemId] : [],
    });
  });
...
function artistSuggestionStatusText() {
  ...
  if (state.artistSuggestions.length) return `建议归属 ${state.artistSuggestions.length} 个`;
  return '未检查';
}
...
function characterSuggestionCoverageWarning(itemIds, tagNames) {
  ...
  const suggestion = (state.characterTagSuggestions || []).find(row => {
    if (!selectedSuggestionNames.has(tagNameKey(row.name))) return false;
    if (!(tagNames || []).some(name => tagNameKey(name) === tagNameKey(row.name))) return false;
    const hitIds = new Set((row.item_ids || []).map(Number));
    return selectedItemIds.filter(id => hitIds.has(Number(id))).length < selectedItemIds.length;
  });
