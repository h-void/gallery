...
pub fn character_recognition_status(conn: &Connection) -> Result<Value> {
    ...
    let (indexed_characters, indexed_references): (i64, i64) = (
        conn.query_row("SELECT COUNT(*) FROM characters", [], |r| r.get(0))
            .unwrap_or(0),
        conn.query_row("SELECT COUNT(*) FROM character_references", [], |r| {
            r.get(0)
        })
        .unwrap_or(0),
    );
    ...
    let session_failed = session
        .get("reason")
        .and_then(|v| v.as_str())
        .is_some_and(|reason| reason == "session_load_failed");
    ...
    Ok(json!({
        ...
        "note": "CCIP via onnxruntime. Providers auto-selected: CUDA (NVIDIA) > OpenVINO (Intel iGPU) > CPU. gpu_access.ready must be true for real Intel iGPU; otherwise CPU fallback. NAS CPU meters still move because preprocess runs on CPU.",
    }))
}
...
pub fn artist_recognition_status() -> Value {
    ...
    let files = ["dino_vits8.onnx", "wd14.onnx"];
...
pub fn recognize_character_native_topk_with_roots(
    conn: &Connection,
    roots: &crate::media_roots::MediaRoots,
    item_id: i64,
    top_k: i64,
) -> Result<Value> {
    crate::character_ccip::recognize_character_native_with_roots(conn, roots, item_id, top_k)
}
...
mod tests {
    ...
    fn status_shapes_without_python() {
        ...
        assert_eq!(st["backend"], "onnxruntime");
